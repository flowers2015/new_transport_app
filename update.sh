...
echo -e "${YELLOW}🔄 در حال اجرای Migration برای افزودن ستون‌های امنیتی به جدول users...${NC}"
node migrations/add_user_security_columns.js

if [ $? -ne 0 ]; then
    echo -e "${YELLOW}⚠️ هشدار: Migration add_user_security_columns اجرا نشد (ممکن است قبلاً اجرا شده باشد)${NC}"
fi

echo -e "${YELLOW}🔄 در حال اجرای Migration برای ستون must_change_password...${NC}"
node migrations/add_must_change_password.js

if [ $? -ne 0 ]; then
    echo -e "${YELLOW}⚠️ هشدار: Migration add_must_change_password اجرا نشد (ممکن است قبلاً اجرا شده باشد)${NC}"
fi
...
echo -e "${YELLOW}🔄 در حال اجرای Migration برای افزودن ستون vehicle_code به جدول vehicles...${NC}"
node migrations/add_vehicle_code_column.js

if [ $? -ne 0 ]; then
    echo -e "${YELLOW}⚠️ هشدار: Migration add_vehicle_code_column اجرا نشد (ممکن است قبلاً اجرا شده باشد)${NC}"
fi
