/**
 * افزودن نقش inspector (بازرسی) به enum نقش کاربران
 */
...
async function run() {
  ...
  try {
    ...
    if (exists.rows.length > 0) {
      console.log('✅ نقش inspector از قبل وجود دارد.');
      return;
    }

    await client.query(`ALTER TYPE user_role_enum ADD VALUE 'inspector'`);
    console.log('✅ نقش inspector (بازرسی) به user_role_enum اضافه شد.');
  } finally {
    client.release();
