...
async function checkDatabase() {
  ...
  try {
    ...
    const [driversCount, vehiclesCount, personalDriversCount, personalVehiclesCount] = await Promise.all([
      ...
      pool.query('SELECT COUNT(*) as count FROM vehicles WHERE is_deleted = false'),
...
async function checkBackendCompression() {
  ...
  return new Promise((resolve) => {
    const options = {
      ...
      path: '/api/v1/personal-drivers',
      method: 'GET',
      headers: {
        'Accept-Encoding': 'gzip, deflate',
        'Authorization': `Bearer ${process.env.TEST_TOKEN || 'test'}` // ممکن است نیاز به token داشته باشد
      }
    };
    ...
    req.on('error', (error) => {
      ...
      console.log('   ⚠️  Backend ممکن است در حال اجرا نباشد یا نیاز به authentication داشته باشد\n');
      resolve();
    });
