...
function sessionStartIntro(stage) {
  if (isVeryFarPublicStage(stage)) {
    return (
      'ابتدا بار خیلی‌دور برای همه نفراتی که این دوره خیلی‌دور نرفته‌اند اعلام می‌شود (نوبت دور و نزدیک).\n' +
      'اگر بار برندارند می‌توانند «بمانم برای مرحله بعد» بزنند.\n' +
      'سپس مسیرهای باقی‌مانده از ابتدای نوبت دور تا انتهای صف اعلام می‌شود.'
    );
  }
  return 'اعلام بار نهایی آغاز شد.\nمسیرهای باقی‌مانده از ابتدای نوبت دور تا ته صف اعلام می‌شود.';
}
...
function namesAfterImageText(stage, namesText) {
  if (isVeryFarPublicStage(stage)) {
    return (
      ...
      'ابتدا به نفراتی که این دوره خیلی‌دور نرفته‌اند (دور و نزدیک) اعلام خواهد شد:\n\n' +
      namesText
    );
  }
  return (
    `${boldTitle('اعلام بار نهایی')}\n` +
    'ترتیب اعلام از ابتدای نوبت دور تا ته صف:\n\n' +
    namesText
  );
}
...
function skippedStage1ToFinal(reason) {
  ...
  if (reason === 'no_far_queue') {
    ...
    return isLegacyTransportQueueLaw()
      ? 'نوبت «دور» خالی است — جلسه از اعلام بار نهایی ادامه می‌یابد.'
      : 'راننده‌ای که این دوره خیلی‌دور نرفته باشد در صف نیست — جلسه از اعلام بار نهایی ادامه می‌یابد.';
  }
