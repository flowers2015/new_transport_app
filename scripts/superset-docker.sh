...
case "$ACTION" in
  up)
    "${COMPOSE[@]}" up -d
    echo "✅ Superset بالا آمد. چند ثانیه صبر کنید سپس:"
    echo "   bash scripts/superset-docker.sh init"
    ;;
  ps)
    "${COMPOSE[@]}" ps
    ;;
  ...
  init)
    ...
    ;;
  *)
    echo "استفاده: bash scripts/superset-docker.sh [up|ps|logs|stop|down|init]"
    exit 1
    ;;
esac
